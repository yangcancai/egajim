...
SCP_SRC_FILE="_build/prod/rel/egajim/egajim-${APP_VERSION}.tar.gz"
PROJ_DIR="/app"
SCP_DST_DIR=/data/egajim
APP=egajim
APP_START="bin/$APP daemon"
APP_PID="bin/$APP pid"
APP_VER="bin/$APP versions"
APP_TAR="tar -xf ${APP}-$APP_VERSION.tar.gz -C egajim"
...
. ./scripts/ssh.sh
cp_static(){
     echo_eval cp -r assets/dist/* apps/egajim/priv/static/
     echo_eval cp -r assets/lib/dist/* apps/egajim/priv/static/
}
cp_doc(){
    echo_eval cp doc/egajim_api.html apps/egajim/priv/static/
}
set_var(){
    # cp_static
    # cp_doc
    export EGAJIM_LOG_ROOT="logs"
	export EGAJIM_LOG_LEVEL="error"
    export EGAJIM_NODE_NAME="egajim@127.0.0.1"
    export EGAJIM_COOKIE="xxx"
    export EGAJIM_EJABBERD_PORT="5222"
    export EGAJIM_EJABBERD_HOST="localhost"
...
release) release $2;;
tar) release $2
     croc send --code xxxx _build/prod/rel/egajim/egajim-*.tar.gz;;
